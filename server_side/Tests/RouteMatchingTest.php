...
final class RouteMatchingTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $collection->addItem(new Currency\Routing\Route('updateUser', '/users/([\w%-]{1,30})', [
            'type' => 'resource',
            'requestMethod' => 'PUT'
        ]));
        $collection->addItem(new Currency\Routing\Route('deleteUser', '/users/([\w%-]{1,30})', [
            'type' => 'resource',
        ...
        $collection->addItem(new Currency\Routing\Route('getUsersCollection', '/users', [
            'type' => 'collection',
            'requestMethod' => 'GET'
        ]));
        $collection->addItem(new Currency\Routing\Route('deleteUsersCollection', '/users', [
            'type' => 'collection',
            'requestMethod' => 'DELETE'
        ]));
        $collection->addItem(new Currency\Routing\Route('aboutMe', '/aboutMe/?', [
            'type' => 'site',
            'requestMethod' => 'GET'
        ]));
        $collection->addItem(new Currency\Routing\Route('pageNotFound', '/404/?', [
            'type' => 'site',
            'requestMethod' => 'GET'
        ]));
        $collection->addItem(new Currency\Routing\Route('index', '/?', [
            'type' => 'site',
            'requestMethod' => 'GET'
        ]));

        $this->routeMatching = new Currency\Routing\RouteMatching($collection->getCollection());
    }
    /**
